...
print(f"Ngal = {len(gals)}, ({np.min(gals['id'])} ~ {np.max(gals['id'])})\tNparts = {len(gpids)}\n")
dprint(f"Ngal = {len(gals)}, ({np.min(gals['id'])} ~ {np.max(gals['id'])})\tNparts = {len(gpids)}\n", logger)
...
for i, gal in enumerate(gals):
    try:
        ...
        elapse, msg = save_hdf(snap, gal, partids, calc_total=True, maxGB=p.maxGB, logger=logger)
        dprint(f"Galaxy #{i+1} of {len(gals)} is saved in {elapse:.2f} sec {msg}", logger)
